...
impl<'a> Screen for MainScreen<'a> {
    fn view(&self, f: &mut Frame<CrosstermBackend<Stderr>>) {
        ...
        let mod_name_block = Block::new()
            .title("Mod name")
            .borders(Borders::ALL)
            .style(self.focus.choose_at(0, HIGHLIGHTED_BLOCK_STYLE, Style::default()));
        ...
        f.render_widget(Paragraph::new("The human-readable name of your mod."), mod_name_layout[0]);
        f.render_widget(self.mod_name_area.widget(), mod_name_layout[1]);

        let mod_id_block = Block::new()
            .title("Mod ID (optional)")
            .borders(Borders::ALL)
            .style(self.focus.choose_at(1, HIGHLIGHTED_BLOCK_STYLE, Style::default()));
        ...
        f.render_widget(Paragraph::new("A unique ID for your mod."), mod_id_layout[0]);
        f.render_widget(self.mod_id_area.widget(), mod_id_layout[1]);

        let package_name_block = Block::new()
            .title("Package name")
            .borders(Borders::ALL)
            .style(self.focus.choose_at(2, HIGHLIGHTED_BLOCK_STYLE, Style::default()));
        ...
        f.render_widget(Paragraph::new("A unique package name for your mod."), package_name_layout[0]);
        f.render_widget(self.package_name_area.widget(), package_name_layout[1]);

        let minecraft_version_block = Block::new()
            .title("Minecraft version")
            .borders(Borders::ALL)
            .style(self.focus.choose_at(3, HIGHLIGHTED_BLOCK_STYLE, Style::default()));
        ...
        f.render_widget(Paragraph::new("The set of names used for Minecraft code."), mappings_layout[0]);
...
pub fn create_app(terminal: Terminal<CrosstermBackend<Stderr>>) -> App {
    ...
    app.push_screen(Box::new(MainScreen::new()));
    app
}
